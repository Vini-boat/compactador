...
void run_test_mode(args_t *args){
    ...
    reader_thread.args = (void *) reader_thread_args_create(args->to_compress_filename, &fifo_read_to_compressor);
    ...
    printf("compressor_thread:\n");
    print_thread_info(&compressor_thread, "\t");
    ...
    if (reader_thread.status != NULL || compressor_thread.status != NULL || writer_thread.status != NULL) {
